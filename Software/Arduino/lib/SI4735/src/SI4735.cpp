...
int16_t SI4735::getDeviceI2CAddress(uint8_t resetPin)
{
    ...
    Serial.println("Wire beginTransmission 0x11");
    ...
    if (error == 0)
    {
        Serial.println("Found device at 0x11");
        setDeviceI2CAddress(0);
        return SI473X_ADDR_SEN_LOW;
    }

    // check 0X63 I2C address
    Serial.println("Wire beginTransmission 0x63");
    ...
    if (error == 0)
    {
        Serial.println("Found device at 0x63");
